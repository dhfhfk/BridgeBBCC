(function () {
...
})()(
	{
		...
		2: [
			function (require, module, exports) {
				...
				configDefault = {
					...
					youtubeHandle: "@Jindol_Fox", // 접속할 유튜브 채널 핸들
				...
				var applyMessageRemove = function (box) {
					...
					if (configData.msgAniDuration) {
						...
						box.timeout = setTimeout(getRemoveTimeout(box), timeToMs(newDuration) || timeToMs(origDuration));
					} else {
						// 메세지 삭제 애니메이션 무시
						getRemoveTimeout(box)();
						return;
				...
				addChatMessage = function (nick, message, data) {
					...
					if (data) {
						...
						if (data.platform) {
							...
							chatBadge.src = `images/badge/${data.platform}.png`
							chatBadge.classList.add(`badge_${data.platform}`);
						...
						if (data.subMonths != undefined) {
							// 후원메세지 그 위에 추가하고
							chatLowerBox.innerHTML = '<div class="chat_subscribe_box">' + replaceMsgFormat(configData.subMonthsMsg, data.subMonths) + "</div>" + chatLowerBox.innerHTML;
						}
						if (data.cheers != undefined) {
							// 비트 메세지도 그 위에 추가하고
							chatLowerBox.innerHTML = '<div class="chat_cheer_box">' + replaceMsgFormat(configData.cheersMsg, data.cheers) + "</div>" + chatLowerBox.innerHTML;
						}
					}
						...
						if (configData.debugLevel == 1) {
							...
							debugLog = function (dat, unConcat) {
								if (unConcat) {
									...
									concatChatMessage("DEBUG", dat, { badges: ["moderator/1"], color: "red", escape: false });
								}
							};
						}
					}
				...
				if (configData.badgeVisible) {
					...
					globalBadgeRequest.open("GET", "https://chat.dhfhfk.com/helix/chat/badges/global/display", true);
									...
									channelBadgeRequest.onreadystatechange = function (evt) {
										if (channelBadgeRequest.readyState == 4) {
											if (channelBadgeRequest.status == 200) {
												...
												debugLog("스트리머 고유 뱃지를 불러오는 데 실패했습니다." + "\n에러 코드 " + channelBadgeRequest.status);
											}
				...
				if (configData.loadCheerImgs) {
					applyCheerIcon = function (message, data) {
						...
						while (matches && matches.match(cheerRegExp) != null) {
							...
							var value = match[3] >= 100 ? (match[3] >= 1000 ? (match[3] >= 5000 ? (match[3] >= 10000 ? (match[3] == 100000 ? 100000 : 10000) : 5000) : 1000) : 100) : 1;
							newMessage += match[1] + '<div class="chat_cheer_text"><img class="cheer_icon" src="./images/cheer/' + match[2] + value + '.gif"/>' + match[3] + "</div> ";
						}
					...
					applyTwitchCon = function (message, data) {
						...
						Object.keys(emotes)
							...
							.forEach(function (emote) {
								var emoteRegExp = new RegExp("( |^)(" + emote.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\(/g, "\\(").replace(/\)/g, "\\)") + ")(\\s|$)");
								var emoteElement = '<img class="twitch_emote" src="' + twitchConsUrlTemplate + emotes[emote] + (configData.loadTwitchv2 ? "/default" : "/static") + '/dark/3.0" />';
				...
				debugLog("유튜브에 접속을 시도합니다.");
				var ytClient = function () {
					ytws = new WebSocket("wss://chat.dhfhfk.com/helix/yt-chat");
					ytws.onopen = () => {
						ytws.send(configData.youtubeHandle);
						debugLog(configData.youtubeHandle + "에 접속했습니다.");
						setInterval(function () {
							ytws.send("heartbeat");
						}, 5000);
					};
					ytws.onmessage = function (res) {
						let message = ""
						let badge = []
						const rawMessage = JSON.parse(res.data);
						if (rawMessage.isOwner) badge.push("broadcaster/1")
						if (rawMessage.isModerator) badge.push("moderator/1")
						message = rawMessage.message.map((msg) => {
							if (msg.text) return msg.text;
							if (!msg.isCustomEmoji) return msg.emojiText;
						})
						addChatMessage(rawMessage.author.name, message.join(" "), {platform: "youtube", badges: badge.join(",")});
					};
					ytws.onclose = function () {
						debugLog("유튜브 채팅 서버와의 연결이 종료되었습니다.<br />" + configData.retryInterval + "초 후 재접속을 시도합니다.");
						setTimeout(function () {
							ytClient();
				...
				/* IRC 클라이언트 설정 */
				var joinCount = 0;
				defaultColors = ["#FF0000", "#0000FF", "#00FF00", "#B22222", "#FF7F50", "#9ACD32", "#FF4500", "#2E8B57", "#DAA520", "#D2691E", "#5F9EA0", "#1E90FF", "#FF69B4", "#8A2BE2", "#00FF7F"];
				...
				var client = function () {
									...
									case "PRIVMSG": // 채팅 수신
										...
										message.split(/\s/).forEach(function (phrase) {
											if (phrase.match(linkRegExp)) {
												if ((configData.clipReplaceMsg || "").length > 0 && phrase.indexOf("twitch") >= 0 && phrase.indexOf("clip") >= 0) {
									...
									case "NOTICE": // 공지 메세지
										switch (twitchArgs["msg-id"]) {
											...
											case "host_on": // 호스팅되었을 때
												debugLog(args[5].slice(0, -1) + " 호스팅 중.\n" + "호스팅중인 채팅의 전송은 지원하지 않습니다.");
												break;
										}
										break;
					...
					ws.onclose = function () {
						debugLog("채팅 서버와의 연결이 종료되었습니다.<br />" + configData.retryInterval + "초 후 재접속을 시도합니다.");
						setTimeout(function () {
							client();
						}, configData.retryInterval * 1000);
					};
		...
		3: [
			function (require, module, exports) {
				var twipMsg = configData && configData.twipCheersMsg ? configData.twipCheersMsg : configData.cheersMsg;
